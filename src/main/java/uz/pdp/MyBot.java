...
public class MyBot extends TelegramLongPollingBot {
    ...
    private void process(User currentUser,Update update) {
        ...
        if (msg.equals("/start")){
            sendMessage.setText("Assalomu Alaykum"+currentUser.getFirstName()+", Botga xush kelibsiz");
    ...
    public InlineKeyboardMarkup getInlineMarkup(User currentUser){
        ...
        if (currentUser.getNewsPage() == 0){
            row3.add(button2);
            row3.add(button3);
        }else if (currentUser.getNewsPage() == 90){
            row3.add(button1);
            row3.add(button2);
        } else {
            row3.add(button1);
        ...
        switch (option) {
            case 1:
                for (User user : DataBase.userList) {
                    System.out.println("FirstName: "+user.getFirstName()+" chat Id: "+user.getChatId());
                }

                System.out.println("Enter chat Id to select ");
                String inputName = new Scanner(System.in).nextLine();

                User selectedUser = null;
                for (User user : DataBase.userList) {
                    if (user.getChatId().equalsIgnoreCase(inputName)){
                        selectedUser = user;
                    }
                }
                ...
                while (true){
                    System.out.println("Typing.. (0=> Back): ");
                    ...
                    selectedUser.getMyMessageList().add(new CustomMessage("\t\t\t\t\t\t\t " + message));
